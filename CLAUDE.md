...
2. `DECISION-BRIEF.md` — the spec. L1–L19 are evidence-locked decisions: build on them,
   never relitigate without new evidence (challenges route through the standing auditor
   via the hackathon session).
...
- Commit per completed task; never commit another person's in-progress files.
