...
|---|---|---|---|---|---|
| 0. Skeleton | 1 | Repo, LICENSE, key-mint flow, committed fixtures, `capabilities.json`, gitleaks CI | `npm start` mints a key cold | **blocked: D1–D5 open** | proposed: marketing session (D5 pending) |
| 1. Ingest | 3 | Upload → job → poll → canonical transcript (stereo happy path + mono fallback layer) | Golden tests against fixtures pass | blocked: Phase 0 | proposed: marketing session (D5 pending) |
